...
class MemoryEditorState:
    def __init__(self, store, scope, project_id=None):
        ...
        if isinstance(draft, dict) and isinstance(draft.get('text'), str):
...
class StrandDialog(QDialog):
    def __init__(self, store, project_id=None, parent=None):
        ...
        row = QHBoxLayout()
        save = QPushButton('Save file'); save.clicked.connect(self.save_current); row.addWidget(save)
    ...
    def change_scope(self, *_):
        ...
        self.message.setText('External edits are checked before saving. A conflicting local draft is kept separately.')
    ...
    def reload_current(self):
        self.state.reload(); self.editor.setPlainText(self.state.text)
