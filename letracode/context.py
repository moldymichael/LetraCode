...
Search hits are passages, not full-file obligations. Use read_file(scope=passage) for focused questions; use whole_file (default) for each file the user asks to read completely. Follow next_read_file when more text is needed; discard line selectors. Offsets include BOM/CRLF. Reread changed source hashes/versions. Only verified exposure supports complete-reading claims; check source_truncated and extraction coverage even at EOF.
