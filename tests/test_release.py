...
def test_portable_archive_preserves_binary_files_and_folder_layout(tmp_path):
    ...
    zipped = module.portable_archive(bundle, tmp_path, "0.5.0")
    with zipfile.ZipFile(zipped) as archive:
        assert set(archive.namelist()) == {
            "LetraCode-0.5.0/LetraCode.exe", "LetraCode-0.5.0/_internal/python311.dll"}
...
def test_source_archives_include_platform_build_inputs_and_are_reproducible(tmp_path):
    ...
    zipped = output / "LetraCode-0.5.0.zip"
    ...
    with tarfile.open(output / "LetraCode-0.5.0.tar.gz") as archive:
        assert set(archive.getnames()) == names
        assert archive.getmember(prefix + "install.sh").mode & 0o111
    if os.name != "nt":
        assert (output / "LetraCode-0.5.0.run").stat().st_mode & 0o111
    subprocess.run(command, check=True, capture_output=True)
    assert first == {file.name: file.read_bytes() for file in output.iterdir()}

    extracted = tmp_path / "Extracted Source/LetraCode-0.5.0"
