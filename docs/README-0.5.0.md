# Historical 0.5.0 README

Preserved as a development record. Use the current README for today’s interface.
...
see [Current implementation state](CURRENT-STATE.md).
...
See [Fine-Tuning setup and workflow](FINE-TUNING.md).
...
pre-install checks](EVALUATION-MEMORY.md) before upgrading an existing data
...
See the consistency boundary in [the reliability report](RELIABILITY-VERIFICATION.md). To remove the application while
...
cannot substitute for this pair. See [Gemma setup and measured verification](GEMMA4.md)
